...
#include "signo.h"
  ...
  void empresa::alquilar_pelicula() {
    ...
    int tiempo_renta;
    float dinero;
    int peliculas_alquiladastotal;
    vector<pair<string,int>>vec;

    if (dni_search(carnet) == -1) { //si el dni no es de algun Usuario ya registrado
      registrar_usuario();
    }
      ...
      peliculas_alquiladastotal++;

      for(int i = 0; i < peliculas_alquiladastotal; i++){
          vec[i].first = carnet;
          vec[i].second = peliagudo;
      }
  ...
  void empresa::reporte_peliculas(){
      ...
      cout<<"Ganancia por peliculas devueltas: not found turron,"<<endl;
      cout<<"Ganancia total de peliculas: not found turron."<<endl;
